...
// Streaming auth proxy for payment evidence/receipt files. serverApi() isn't
// reused here since it parses the response body as JSON; this route needs
// the raw bytes/headers instead. PaymentEvidenceFile.downloadUrl from the
// backend is never used directly by the client — every consumer builds this
// same BFF-relative path from (projectId, fileId) instead, so a leaked link
// can't bypass auth.
export async function GET(
  ...
  try {
    const upstream = await fetch(`${API_URL}/projects/${id}/files/payments/${fileId}`, {
    ...
    console.error("GET /api/projects/[id]/files/payments/[fileId] error:", error);
